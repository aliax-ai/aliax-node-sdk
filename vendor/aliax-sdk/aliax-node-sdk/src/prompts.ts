 ...
 *
 * Verbatim mirror of `aliax/prompts.py::ALIAX_SYSTEM_INSTRUCTIONS`. The
 * Python wheel is the reference: when a new verb lands in `execute()` we
 * bump this string + the matching branch in client.ts in the same commit
 * so customers get "invisible version upgrades" via `npm i -U`.
 *
 * V1 action catalog (Lean — ruthlessly trimmed):
 *   CLICK, TYPE, TYPE_AND_ENTER, HOVER, SCROLL_{DOWN,UP,LEFT,RIGHT},
 *   WAIT, BATCH_TYPE, COMBO, REPORT_ISSUE.
 *
...
export const ALIAX_SYSTEM_INSTRUCTIONS = `=== ALIAX VISUAL NAVIGATION PROTOCOL ===
...
export const SYSTEM_INSTRUCTIONS = ALIAX_SYSTEM_INSTRUCTIONS;
